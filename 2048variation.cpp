...
//Software Engineering Assignment 3 Variation Spring 2015
//Francine Tran 00953181


using namespace std;

bool shiftGrid(vector <int> &game,string move ,int &score);
...
int main(){
	...
	cout<<"Please enter a savefile."<<endl;
	cin>>gamefile;
	...
	cout << "Your possible moves are w,s,a,d. Or you can type save to save your current game."<<endl;
	do{
		cin>>move;
		if(move!="save"){

			if(checkforspaces(game)){
				if(shiftGrid(game,move,score)){
					if(checkforspaces(game)){
					randomTwo(game);
					}
				writeGrid(game);
			...
			outfile<<score<<endl;
			outfile.close();
			cout <<"Your game has been saved under the filename: savefile.txt"<<endl;
		}
